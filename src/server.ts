...
io.on("connection", (socket: Socket<ClientToServerEvents, ServerToClientEvents>) => {
  ...
  socket.on("comprouSaldoDentroConsulta", (data) => {
    io.sockets.to(data.room).emit("atualizarAdm")
  })
...
server.post("/apagarProfissional", confereTokenAdmGeral, async (req: Request, res: Response) => {
  ...
  if(idProfissionalApagar){
    try{
      await db("loginatendentes").where("id_profissional", idProfissionalApagar).del()
      await db("profissionais").where("id", idProfissionalApagar).del()
  ...
  try{
    ...
    if(arrPagamentoExistente.length > 0){
      ...
      payment.create({ body, requestOptions: { idempotencyKey: req.body.idempotencyKey} }).then(async (result) => {
        ...
        if(result.api_response.status !== 400 && result.api_response.status !== 500){
          res.json(["sucesso", result])
        }else{
          return res.json(["erro", result])
        }
